...
from utils.api.router import select_model, generate_answer_with_model, deepseek_chat_completion
...
async def extract_search_keywords(user_query: str, nvidia_rotator) -> List[str]:
    """Extract intelligent search keywords from user query using DeepSeek agent."""
...
Return only the keywords, separated by spaces, no other text."""
        ...
        # Use DeepSeek for better keyword extraction
...
Return as JSON array of objects."""
        ...
        # Use DeepSeek for better strategy generation
        response = await deepseek_chat_completion(sys_prompt, user_prompt, nvidia_rotator)
...
async def extract_relevant_content(content: str, user_query: str, nvidia_rotator) -> str:
    """Use DeepSeek agent to extract only the content relevant to the user query."""
    if not nvidia_rotator:
...
Return only the relevant content, no additional commentary."""
        ...
        # Use DeepSeek for better content extraction
        response = await deepseek_chat_completion(sys_prompt, user_prompt, nvidia_rotator)
...
async def assess_content_quality(content: str, nvidia_rotator) -> Dict[str, Any]:
    """Assess content quality using DeepSeek agent."""
...
Consider: accuracy, completeness, clarity, authority, recency, bias, factual claims."""
        ...
        # Use DeepSeek for better quality assessment
        response = await deepseek_chat_completion(sys_prompt, user_prompt, nvidia_rotator)
...
Focus on factual claims, statistics, and verifiable information."""
        ...
        # Use DeepSeek for better cross-validation
        response = await deepseek_chat_completion(sys_prompt, user_prompt, nvidia_rotator)
...
Be clear and direct."""
        ...
        # Use DeepSeek for better summarization
        response = await deepseek_chat_completion(sys_prompt, user_prompt, nvidia_rotator)
